...
def objective(trial):
    ...
    #hold_period = trial.suggest_int('hold_period', 5, 8, step = 1)
    hold_period = 8
        ...
        for chg_date in all_tensors_dict.keys():
            ...
            if chg_date == fst_select_date:
                ...
                if long_logits.shape[0] != 1 and short_logits.shape[0] != 1:
                    ...
                    long_logits[dropped_long_indices] = -1
                    short_logits[dropped_short_indices] = -1
                    select_params['long']['logits'] = long_logits
                    select_params['short']['logits'] = short_logits
                    select_params['long']['new_cnt'] = new_long_cnt
                    select_params['short']['new_cnt'] = new_short_cnt
...
class Trader(nn.Module):
    ...
    def forward(self, state, liq_tensor):
        ...
        probs = exp(log_softmax(output.T.squeeze(0), dim = 0))
        probs_liq = twhere(liq_tensor == 0, 0, probs)
        pure_probs = tdiv(probs_liq, probs_liq.sum())
        max_val = pure_probs.max()
        #print('max val:', max_val)
        #print('Shape of pure probs:', pure_probs.shape)
        probs_detach = pure_probs.detach()
        if (max_val == 0) or (not tisfinite(probs_detach).all()):
            return self.err_flag
        else:
            return twhere(liq_tensor == 0, -1, pure_probs)
...
if __name__ == '__main__':
    ...
    dbx_study_fname = f'/qnt_Adamax_lowpct_strict_attn.pkl'
    local_study_fname = f'{folder}{dbx_study_fname}'
    dbx_params_fname = f'/params_Adamax_lowpct_strict_attn.csv'
    local_params_fname = f'{folder}{dbx_params_fname}'
    dbx_df_fname = f'/qnt_Adamax_lowpct_strict_attn.csv'
    local_df_fname = f'{folder}{dbx_df_fname}'
    dbx_wt_fname = f'/wt_Adamax_lowpct_strict_attn.csv'
    local_wt_fname = f'{folder}{dbx_wt_fname}'
    dbx_long_model_fname = f'/long_model_Adamax_lowpct_strict_attn.pt'
    local_long_model_fname = f'{folder}{dbx_long_model_fname}'
    dbx_short_model_fname = f'/short_model_Adamax_lowpct_strict_attn.pt'
    ...
    if download_val != 1:
        ...
        best_trial_num, trial_cnt = study.best_trial.number, 20
