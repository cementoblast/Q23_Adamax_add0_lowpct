...
def objective(trial):
    ...
    long_min_days = trial.suggest_int("long_min_days", 10, 200, step = 10)
    ...
    long_model = Trader(hidden_size, batch_mom = batch_mom, actor_act = getattr(nn, long_actor))
    short_model = Trader(hidden_size, batch_mom = batch_mom, actor_act = getattr(nn, short_actor))
...
class Trader(nn.Module):
    def __init__(self, hidden_size: int, batch_mom: float, actor_act: nn):
        ...
        self.common_l1 = nn.Linear(32, hidden_size)
        ...
        self.critic_model = nn.Sequential(
            self.critic_act2, self.critic_l3, nn.AlphaDropout(p = 0.3), self.critic_tanh4)
...
if __name__ == '__main__':
    ...
    if not isdir(folder):
        mkdir(folder)
    st_time = time()
    start_date, end_date, prepare_date = '2006-01-01', '2025-05-21', '2005-03-18'
    ...
    dbx_study_fname = f'/qnt_Adamax_lowpct_strict_newmodel2.pkl'
    local_study_fname = f'{folder}{dbx_study_fname}'
    dbx_params_fname = f'/params_Adamax_lowpct_strict_newmodel2.csv'
    local_params_fname = f'{folder}{dbx_params_fname}'
    dbx_df_fname = f'/qnt_Adamax_lowpct_strict_newmodel2.csv'
    local_df_fname = f'{folder}{dbx_df_fname}'
    dbx_wt_fname = f'/wt_Adamax_lowpct_strict_newmodel2.csv'
    local_wt_fname = f'{folder}{dbx_wt_fname}'
    dbx_long_model_fname = f'/long_model_Adamax_lowpct_strict_newmodel2.pt'
    local_long_model_fname = f'{folder}{dbx_long_model_fname}'
    dbx_short_model_fname = f'/short_model_Adamax_lowpct_strict_newmodel2.pt'
    ...
    if download_val != 1:
        ...
        best_trial_num, trial_cnt = study.best_trial.number, 60
