...
class Trader(nn.Module):
    def __init__(self, actor_act: nn, d_model: int, nhead: int, num_layers: int):
        ...
        self.cont_projection_l1 = nn.Linear(15, d_model)
        self.hidden_size = d_model // 2
        self.l2 = nn.Linear(d_model, self.hidden_size)
        ...
        if isinstance(actor_act(), (nn.Sigmoid, nn.Tanh, nn.Softshrink, nn.Softsign, nn.Tanhshrink)):
            ...
            nn.init.kaiming_uniform_(self.l2.weight, nonlinearity = 'leaky_relu')
    def forward(self, state, liq_tensor):
        x = self.cont_projection_l1(state)  # (n_stocks, d_model-21)
...
if __name__ == '__main__':
    ...
    dbx_study_fname = f'/qnt_Adamax_lowpct_strict_attn_ret.pkl'
    local_study_fname = f'{folder}{dbx_study_fname}'
    dbx_params_fname = f'/params_Adamax_lowpct_strict_attn_ret.csv'
    local_params_fname = f'{folder}{dbx_params_fname}'
    dbx_df_fname = f'/qnt_Adamax_lowpct_strict_attn_ret.csv'
    local_df_fname = f'{folder}{dbx_df_fname}'
    dbx_wt_fname = f'/wt_Adamax_lowpct_strict_attn_ret.csv'
    local_wt_fname = f'{folder}{dbx_wt_fname}'
    dbx_long_model_fname = f'/long_model_Adamax_lowpct_strict_attn_ret.pt'
    local_long_model_fname = f'{folder}{dbx_long_model_fname}'
    dbx_short_model_fname = f'/short_model_Adamax_lowpct_strict_attn_ret.pt'
    ...
    #last_data = all_tensors_dict[last_train_date]
    #print('Last data shape:', last_data.shape)
