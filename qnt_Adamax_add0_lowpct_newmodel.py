...
from numpy import nanmedian, maximum, cumprod, count_nonzero, unique, dot, corrcoef, log, arange, isnan, nanmax, nanmin, NaN, zeros, clip, where, float32, isfinite, array, append, NINF, intersect1d, setdiff1d, union1d
...
def objective(trial):
    ...
    long_model = Trader(hidden_size, batch_mom, actor_act = getattr(nn, long_actor), critic_act = getattr(nn, long_critic), sector_dim = sector_dim, weekday_dim = weekday_dim, month_dim = month_dim)
    short_model = Trader(hidden_size, batch_mom, actor_act = getattr(nn, short_actor), critic_act = getattr(nn, short_critic), sector_dim = sector_dim, weekday_dim = weekday_dim, month_dim = month_dim)
    ...
    return process(long_model, long_opt, short_model, short_opt, total_epochs, hold_period, long_cnt, short_cnt, update_cnt, stk_pct, long_lgbm, short_lgbm, long_upper, long_lower, short_upper, short_lower)

def test_model(long_model, short_model, long_cnt, short_cnt, stk_pct, long_lgbm, short_lgbm, long_upper, long_lower, short_upper, short_lower):
    long_model.eval()
    short_model.eval()
    ...
    wts_np[:] = NaN
    #atr_df = qnta.atr(high, low, prices, 14).to_pandas()
    ...
    with no_grad():
        for chg_date in all_tensors_dict.keys():
            ...
            if chg_date == fst_select_date:
                ...
                hold_long_indices, hold_short_indices = long_model.hold_indices.astype(int), short_model.hold_indices.astype(int)
                #stop_long_cnt, stop_short_cnt = long_cnt - len(long_names), short_cnt - len(short_names)
                ...
                if valid_long_indices.shape[0] > 0:
                    long_stop_input = final_df.loc[assets[valid_long_indices], :]
                    pred_upper = long_upper.predict(long_stop_input)
                    pred_lower = long_lower.predict(long_stop_input)
                    dropped_long_indices = drop_indices(valid_long_indices, pred_upper, pred_lower).astype(int)
                    long_model.hold_indices = setdiff1d(hold_long_indices, dropped_long_indices, assume_unique = True)
                    wts_np[chg_date_idx, dropped_long_indices] = 0
                else:
                    dropped_long_indices = []
                if valid_short_indices.shape[0] > 0:
                    short_stop_input = final_df.loc[assets[valid_short_indices], :]
                    pred_upper = short_upper.predict(short_stop_input)
                    pred_lower = short_lower.predict(short_stop_input)
                    dropped_short_indices = drop_indices(valid_short_indices, pred_upper, pred_lower).astype(int)
                    short_model.hold_indices = setdiff1d(hold_short_indices, dropped_short_indices, assume_unique = True)
                    wts_np[chg_date_idx, dropped_short_indices] = 0
                else:
                    dropped_short_indices = []
                if illiq_long_indices.shape[0] > 0:
                    wts_np[chg_date_idx, illiq_long_indices] = 0
                    long_model.hold_indices = setdiff1d(long_model.hold_indices, illiq_long_indices, assume_unique = True)
                if illiq_short_indices.shape[0] > 0:
                    wts_np[chg_date_idx, illiq_short_indices] = 0
                    short_model.hold_indices = setdiff1d(short_model.hold_indices, illiq_short_indices, assume_unique = True)
                new_long_cnt, new_short_cnt = len(dropped_long_indices) + len(illiq_long_indices), len(dropped_short_indices) + len(illiq_short_indices)
            long_policy = long_model(final_tensor, liq_tensor)[0]
            short_policy = short_model(final_tensor, liq_tensor)[0]
            long_stks, short_stks = array([], dtype = int), array([], dtype = int)
            if type(long_policy) != int and type(short_policy) != int:
                long_logits = long_policy.view(-1)
                short_logits = short_policy.view(-1)
                hold_long_indices, hold_short_indices = long_model.hold_indices, short_model.hold_indices
                if len(hold_long_indices) != 0:
                    long_stks = append(long_stks, hold_long_indices)
                    long_logits[hold_long_indices] = -1
                    short_logits[hold_long_indices] = -1
                if len(hold_short_indices) != 0:
                    short_stks = append(short_stks, hold_short_indices)
                    long_logits[hold_short_indices] = -1
                    short_logits[hold_short_indices] = -1
                if new_long_cnt > 0:
                    long_indices = topk(long_logits, k = new_long_cnt, largest = True, sorted = False)[1].numpy()
                    long_stks = append(long_stks, long_indices).astype(int)
                if new_short_cnt > 0:
                    short_indices = topk(short_logits, k = new_short_cnt, largest = True, sorted = False)[1].numpy()
                    short_stks = append(short_stks, short_indices).astype(int)

                common_stks = intersect1d(long_stks, short_stks, assume_unique = True).astype(int)
                if common_stks.shape[0] > 0:
                    ...
                    long_diff = setdiff1d(long_stks, common_stks, assume_unique = True)
                    short_diff = setdiff1d(short_stks, common_stks, assume_unique = True)
                    long_stks = append(long_diff, final_long_stks).astype(int)
                    short_stks = append(short_diff, final_short_stks).astype(int)
                    long_vacants, short_vacants = long_cnt - long_stks.shape[0], short_cnt - short_stks.shape[0]
                    if short_vacants > 0:
                        short_logits[long_stks] = -1
                        short_logits[short_stks] = -1
                        new_short = topk(short_logits, k = short_vacants, largest = True, sorted = False)[1].numpy()
                        short_stks = append(short_stks, new_short).astype(int)
                        long_logits[new_short] = -1
                    if long_vacants > 0:
                        long_logits[long_stks] = -1
                        long_logits[short_stks] = -1
                        new_long = topk(long_logits, k = long_vacants, largest = True, sorted = False)[1].numpy()
                        long_stks = append(long_stks, new_long).astype(int)
                long_model.hold_indices = long_stks
                short_model.hold_indices = short_stks
                wts_np[chg_date_idx, long_stks] = stk_pct
                wts_np[chg_date_idx, short_stks] = -stk_pct
            else:
...
def drop_indices(all_indices, pred_upper, pred_lower):
    pos_indices = where(pred_upper > 0)[0]
    if len(pos_indices) > 0:
        sel_indices01 = intersect1d(pos_indices, where(pred_lower > 0)[0], assume_unique = True)
        sel_indices02 = where(pred_upper > -pred_lower)[0]
        return setdiff1d(all_indices, all_indices[union1d(sel_indices01, sel_indices02)], assume_unique = True)
    else:
        return all_indices
...
class Trader(nn.Module):
    def __init__(self, hidden_size: int, batch_mom: float, actor_act: nn, critic_act: nn, sector_dim: int, weekday_dim: int, month_dim: int):
        super(Trader, self).__init__()
        self.emb_sector = nn.Embedding(9, sector_dim)
        self.emb_month = nn.Embedding(12, month_dim)
        self.emb_weekday = nn.Embedding(5, weekday_dim)
        self.total_dim = sector_dim + weekday_dim + month_dim + 15
        self.common_l1 = nn.Linear(self.total_dim, hidden_size)
        self.actor_act2 = actor_act()
        self.actor_l3 = nn.Linear(hidden_size, 1)
        #self.critic_l1 = nn.Linear(1, 4)
        self.critic_act2 = critic_act()
        self.critic_l3 = nn.Linear(hidden_size, 1)
        self.critic_tanh4 = nn.Tanh()
        for layer in (self.common_l1, self.actor_l3, self.critic_l3):
            for name, param in layer.named_parameters():
                if 'weight' in name:
                    nn.init.xavier_uniform_(param, gain=1)
                else:
                    nn.init.constant_(param, 0)
        self.actor_model = nn.Sequential(nn.BatchNorm1d(hidden_size, momentum = batch_mom),
            self.actor_act2, nn.AlphaDropout(p = 0.3), self.actor_l3)
        self.critic_model = nn.Sequential(
            self.critic_act2, nn.AlphaDropout(p = 0.3), self.critic_l3, self.critic_tanh4)
        self.hold_indices = array([])
    def forward(self, state, liq_tensor):
        ...
        actor_x = self.common_l1(x)
        critic_x = actor_x.detach()
        a3 = self.actor_model(actor_x)
        critic = self.critic_model(critic_x.mean(dim = 0))
        if critic.shape[0] != 1:
            raise ValueError(f'critic.shape[0] != 1:\nCritic val:{critic}, critic shape: {critic.shape}')
        a4 = exp(log_softmax(a3.T.squeeze(0), dim = 0))
        actor = twhere(liq_tensor == 0, 0, a4)
        pure_actor = actor / actor.sum()
        ...
        if max_val == 0:
            ...
            print('a4:', a4)
            return 1, 1
        elif not isfinite(actor_detach).all():
            print('a4 is not finite!!!!!!')
            print('a4:', a4)
            return 1, 1
        else:
...
if __name__ == '__main__':
    ...
    if not isdir(folder):
        mkdir(folder)
    st_time = time()
    start_date, end_date, prepare_date = '2006-01-01', '2025-05-14', '2005-03-18'
    ...
    dbx_study_fname = f'/qnt_Adamax_add0_lowpct_newmodel.pkl'
    local_study_fname = f'{folder}{dbx_study_fname}'
    dbx_params_fname = f'/params_Adamax_add0_lowpct_newmodel.csv'
    local_params_fname = f'{folder}{dbx_params_fname}'
    dbx_df_fname = f'/qnt_Adamax_add0_lowpct_newmodel.csv'
    local_df_fname = f'{folder}{dbx_df_fname}'
    dbx_wt_fname = f'/wt_Adamax_add0_lowpct_newmodel.csv'
    local_wt_fname = f'{folder}{dbx_wt_fname}'
    dbx_long_model_fname = f'/long_model_Adamax_add0_lowpct_newmodel.pt'
    local_long_model_fname = f'{folder}{dbx_long_model_fname}'
    dbx_short_model_fname = f'/short_model_Adamax_add0_lowpct_newmodel.pt'
    ...
    def train(model, opt, trade_type: int, hold_period: int, update_cnt: int, return_dict: dict):
        ...
        for fst_date in true_train_dates:
            ...
            #twrr = cumprod(daily_ratio, axis = 0)[-1, :] ** (1 / hold_days) - 1
            #pass_ret_std = nanstd(daily_pct, axis = 0, ddof = 1)
            #rew = mean(twrr / pass_ret_std) * annual_coef
            """
            if not isfinite(rew).all():
                print('Daily pct:\n', daily_pct)
                print('Daily ratio:\n', daily_ratio)
                print('Daily val:\n', daily_val)
                print('Max val:\n', max_val)
                print('MDD val:\n', mdd_val)
                print('Median sharpe:\n', rew)
                raise ValueError(f'Median sharpe is not finite!!!!!!')
            """
            #tmp_wts_df.iloc[start_idx: end_idx, :] = tmp_wts[start_idx: end_idx, :]
            #tmp_wts_df.iloc[start_idx: end_idx, :] = tmp_wts_df.iloc[start_idx: end_idx, :].ffill(axis = 0) * mon_liq
            #tmp_wts_df.loc[:, assets] = tmp_wts_df.loc[:, assets].astype(float)
            #stat_wts = tmp_wts_df.copy(deep = True)
            #stat_wts.set_index('time', inplace = True)
            #wts_xr = tmp_wts_df.unstack().to_xarray()
            #stats = qnstats.calc_stat(data, wts_xr)
            #stats_df = stats.to_pandas()
            #stats_df.reset_index(inplace = True)
            #stats_df = stats_df[(stats_df['time'] >= fst_date) & (stats_df['time'] < next_date)]
            #stats_df.reset_index(inplace = True, drop = True)
            #rew = stats_df.loc[last_date, 'sharpe_ratio']
            stat_end_time = time()
    ...
    def process(long_model, long_opt, short_model, short_opt, total_epochs, hold_period, long_cnt, short_cnt, update_cnt, stk_pct, long_lgbm, short_lgbm, long_upper, long_lower, short_upper, short_lower):
        #long_model, long_opt, short_model, short_opt, total_epochs, hold_period, long_cnt, short_cnt, update_cnt, stk_pct, long_lgbm, short_lgbm, long_upper, long_lower, short_upper, short_lower
            ...
            if not stop:
                ...
                with ctx.Manager() as manager:
                    ...
                    if type(long_result) != int and type(short_result) != int:
                        ...
                        st_test_time = time()
                        sharpe, arr, mdd, volat, calmar, stats_df, wts_df = test_model(long_model, short_model, long_cnt, short_cnt, stk_pct, long_lgbm, short_lgbm, long_upper, long_lower, short_upper, short_lower)
                        end_test_time = time()
    ...
    date_indices = {date_: npnz(all_dates == date_)[0][0] for date_ in stk_select_dates}
    ...
    if download_val != 1:
        ...
        best_trial_num, trial_cnt = study.best_trial.number, 50
