...
from torch.nn.functional import log_softmax, normalize, softmax
...
def objective(trial):
    ...
    n_head = 2 ** trial.suggest_int("n_head", 2, 4, step = 1)
    n_layer = trial.suggest_int("n_layer", 3, 5, step = 1)
    total_epochs = 6
    stk_cnt_lt = [18, 24, 30, 36, 45]
    ...
    hold_period = trial.suggest_int('hold_period', 2, 8, step = 2)
    ...
    with no_grad():
        for chg_date in all_tensors_dict.keys():
            ...
            if chg_date == fst_select_date:
                ...
                long_logits = long_model(final_tensor, liq_tensor)
                short_logits = short_model(final_tensor, liq_tensor)
                if type(long_logits) != int and type(short_logits) != int:
...
class Trader(nn.Module):
    def __init__(self, actor_act: nn, d_model: int, nhead: int, num_layers: int):
        ...
        self.l3 = nn.Linear(d_model, d_model // 2)
        self.l4 = nn.Linear(d_model // 2, 1)
    ...
    def forward(self, state, liq_tensor):
        ...
        pure_probs = probs_liq / probs_liq.sum()
        ...
        if max_val == 0:
            ...
            return 1
        elif not tisfinite(probs_detach).all():
            print('Probs is not finite')
            return 1
        else:
...
if __name__ == '__main__':
    ...
    dbx_study_fname = f'/qnt_Adamax_lowpct_strict_simple.pkl'
    local_study_fname = f'{folder}{dbx_study_fname}'
    dbx_params_fname = f'/params_Adamax_lowpct_strict_simple.csv'
    local_params_fname = f'{folder}{dbx_params_fname}'
    dbx_df_fname = f'/qnt_Adamax_lowpct_strict_simple.csv'
    local_df_fname = f'{folder}{dbx_df_fname}'
    dbx_wt_fname = f'/wt_Adamax_lowpct_strict_simple.csv'
    local_wt_fname = f'{folder}{dbx_wt_fname}'
    dbx_long_model_fname = f'/long_model_Adamax_lowpct_strict_simple.pt'
    local_long_model_fname = f'{folder}{dbx_long_model_fname}'
    dbx_short_model_fname = f'/short_model_Adamax_lowpct_strict_simple.pt'
    ...
    if download_val != 1:
        ...
        best_trial_num, trial_cnt = study.best_trial.number, 1
