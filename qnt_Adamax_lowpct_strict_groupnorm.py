...
def objective(trial):
    ...
    long_group_cnt = 2 ** trial.suggest_int('long_group_power', 2, 6, step = 1)
    short_group_cnt = 2 ** trial.suggest_int('short_group_power', 2, 6, step = 1)
    ...
    long_model = Trader(hidden_size, actor_act = getattr(nn, long_actor), group_cnt = long_group_cnt)
    short_model = Trader(hidden_size, actor_act = getattr(nn, short_actor), group_cnt = short_group_cnt)
...
class Trader(nn.Module):
    def __init__(self, hidden_size: int, actor_act: nn, group_cnt: int):
        ...
        self.actor_model = nn.Sequential(self.actor_l1,
            nn.GroupNorm(num_groups = group_cnt, num_channels = hidden_size, affine = True), self.actor_act2, nn.AlphaDropout(p = 0.3), self.actor_l3)
...
if __name__ == '__main__':
    ...
    dbx_study_fname = f'/qnt_Adamax_lowpct_strict_groupnorm.pkl'
    local_study_fname = f'{folder}{dbx_study_fname}'
    dbx_params_fname = f'/params_Adamax_lowpct_strict_groupnorm.csv'
    local_params_fname = f'{folder}{dbx_params_fname}'
    dbx_df_fname = f'/qnt_Adamax_lowpct_strict_groupnorm.csv'
    local_df_fname = f'{folder}{dbx_df_fname}'
    dbx_wt_fname = f'/wt_Adamax_lowpct_strict_groupnorm.csv'
    local_wt_fname = f'{folder}{dbx_wt_fname}'
    dbx_long_model_fname = f'/long_model_Adamax_lowpct_strict_groupnorm.pt'
    local_long_model_fname = f'{folder}{dbx_long_model_fname}'
    dbx_short_model_fname = f'/short_model_Adamax_lowpct_strict_groupnorm.pt'
