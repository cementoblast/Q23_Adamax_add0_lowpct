from pandas import options, Series, concat, set_option, read_csv, errors, DataFrame, to_datetime, Grouper, to_numeric, Timestamp
        ...
        for chg_date in all_tensors_dict.keys():
            ...
            if chg_date == fst_select_date:
                ...
                if type(long_policy) != int and type(short_policy) != int:
                    ...
                    new_long_stks, new_short_stks = assets[new_long_indices], assets[new_short_indices]
                    LongSys = TradeSystem(new_long_stks, 'long', 'None', 0, stk_pct, longloss_N, longgain_N, 1, long_upper, long_lower, strict_long, long_min_days)
                    ShortSys = TradeSystem(new_short_stks, 'short', 'None', 0, -stk_pct, shortloss_N, shortgain_N, 1, short_upper, short_lower, strict_short, short_min_days)
                    #initial_stks, trade_type, add_type, max_add_cnt, stk_pct, loss_N, gain_N, add_N, rf_upper, rf_lower, strict, min_days
                    LongSys.hold_df.loc[new_long_stks, 'cost'] = prices_np[chg_date_idx, new_long_indices]
                    ShortSys.hold_df.loc[new_short_stks, 'cost'] = prices_np[chg_date_idx, new_short_indices]
                    LongSys.hold_df.loc[new_long_stks, 'index'] = new_long_indices
                    ShortSys.hold_df.loc[new_short_stks, 'index'] = new_short_indices
                    wts_np[chg_date_idx, new_long_indices] = stk_pct
                    wts_np[chg_date_idx, new_short_indices] = -stk_pct
                else:
                    return 1, 1, 1, 1, 1, 1, 1
            else:
                LongSys.hold_df['days'] += 1
                ShortSys.hold_df['days'] += 1
                long_names, short_names = list(LongSys.hold_df.index), list(ShortSys.hold_df.index)
                hold_long_indices, hold_short_indices = LongSys.hold_df['index'].astype(int), ShortSys.hold_df['index'].astype(int)
                all_hold_indices = append(hold_long_indices, hold_short_indices)
                #raw_indices = union1d(all_hold_indices, nonliq_indices)
                #excluded_indices = raw_indices.view('int64')
                #excluded_indices[:] = raw_indices
                ...
                if valid_long_indices.shape[0] > 0:
                    ...
                    wts_np[chg_date_idx, LongSys.hold_df.loc[dropped_long_names, 'index']] = 0
                    LongSys.hold_df.drop(dropped_long_names, axis = 0, inplace = True)
                else:
                    dropped_long_names = []
                if valid_short_indices.shape[0] > 0:
                    dropped_short_names = ShortSys.pred_quantile(final_df.loc[assets[valid_short_indices], :])
                    wts_np[chg_date_idx, ShortSys.hold_df.loc[dropped_short_names, 'index']] = 0
                    ShortSys.hold_df.drop(dropped_short_names, axis = 0, inplace = True)
                else:
                    dropped_short_names = []
                if illiq_long_indices.shape[0] > 0:
                    wts_np[chg_date_idx, illiq_long_indices] = 0
                    illiq_long_names = assets[illiq_long_indices]
                    LongSys.hold_df.drop(illiq_long_names, axis = 0, inplace = True)

                if illiq_short_indices.shape[0] > 0:
                    wts_np[chg_date_idx, illiq_short_indices] = 0
                    illiq_short_names = assets[illiq_short_indices]
                    ShortSys.hold_df.drop(illiq_short_names, axis = 0, inplace = True)
                ...
                if new_long_cnt > long_cnt:
                    ...
                    print('Long names:', long_names, len(long_names))
                    print('Dropped long names:', dropped_long_names, len(dropped_long_names))
                    print('Illiq long names:', illiq_long_names, len(illiq_long_names))
                ...
                if new_short_cnt > short_cnt:
                    ...
                    print('Short names:', short_names, len(short_names))
                    print('Dropped short names:', dropped_short_names, len(dropped_short_names))
                    print('Illiq short names:', illiq_short_names, len(illiq_short_names))
                ...
                if type(long_policy) != int and type(short_policy) != int:
                    ...
                    if new_long_indices.shape[0] > 0:
                        new_long_stks = assets[new_long_indices]
                        new_long_df = DataFrame(0.00, columns = ['cost', 'last_pr', 'atr', 'N', 'add_cnt', 'days', 'index'], index = new_long_stks, dtype = 'float64')
                        new_long_df.loc[new_long_stks, 'cost'] = prices_np[chg_date_idx, new_long_indices]
                        new_long_df['index'] = new_long_indices
                        new_long_df['index'] = new_long_df['index'].astype('int64')
                        LongSys.hold_df = concat([LongSys.hold_df, new_long_df], axis = 0)
                        if len(LongSys.hold_df.index) > long_cnt:
                            print('New long stks:', new_long_stks, len(new_long_stks))
                            print('Initial hold long names:', long_names, len(long_names))
                            raise ValueError('Current Long cnt:', len(LongSys.hold_df.index), 'Default Long cnt:', long_cnt)
                    if new_short_indices.shape[0] > 0:
                        new_short_stks = assets[new_short_indices]
                        new_short_df = DataFrame(0.00, columns = ['cost', 'last_pr', 'atr', 'N', 'add_cnt', 'days', 'index'], index = new_short_stks, dtype = 'float64')
                        new_short_df.loc[new_short_stks, 'cost'] = prices_np[chg_date_idx, new_short_indices]
                        new_short_df['index'] = new_short_indices
                        new_short_df['index'] = new_short_df['index'].astype('int64')
                        ShortSys.hold_df = concat([ShortSys.hold_df, new_short_df], axis = 0)
                        if len(ShortSys.hold_df.index) > short_cnt:
                            print('New short stks:', new_short_stks, len(new_short_stks))
                            print('Initial hold short names:', short_names, len(short_names))
                            raise ValueError('Current Short cnt:', len(ShortSys.hold_df.index), 'Default Short cnt:', short_cnt)
                    wts_np[chg_date_idx, LongSys.hold_df['index']] = stk_pct
                    wts_np[chg_date_idx, ShortSys.hold_df['index']] = -stk_pct
                else:
                    return 1, 1, 1, 1, 1, 1, 1
...
class BestTrial():
    def __init__(self):
        ...
        self.stat_time_lt = []
class TradeSystem():
    def __init__(self, initial_stks, trade_type, add_type, max_add_cnt, stk_pct, loss_N, gain_N, add_N, rf_upper, rf_lower, strict, min_days):
        ...
        self.hold_df = DataFrame(0.00, columns = ['cost', 'last_pr', 'atr', 'N', 'add_cnt', 'days', 'index'], index = initial_stks, dtype = 'float64')
        self.hold_df['index'] = self.hold_df['index'].astype('int64')
        if strict:
    ...
    def Monitor(self, date_idx, atr_np, prices_np, wts_np):
        if self.hold_df.shape[0] > 0:
            hold_stks = self.hold_df.index
            stk_indices = self.hold_df['index']
            self.hold_df.loc[hold_stks, 'last_pr'] = prices_np[date_idx, stk_indices]
            self.hold_df.loc[hold_stks, 'atr'] = atr_np[date_idx, stk_indices]
            self.hold_df.loc[self.hold_df['atr'] == 0, 'atr'] = 1e-6
            self.hold_df.loc[hold_stks, 'N'] = self.cal_N()
            loss_cond = self.hold_df['N'] <= self.loss_N
            gain_cond = self.hold_df['N'] >= self.gain_N
            min_cond = self.hold_df['days'] >= self.min_days
            stop_stks = self.hold_df[(loss_cond | gain_cond) & (min_cond)].index
            if len(stop_stks) > 0:
                wts_np[date_idx, self.hold_df.loc[stop_stks, 'index']] = 0
                wts_np[date_idx, self.hold_df.loc[setdiff1d(hold_stks, stop_stks), 'index']] = self.stk_pct
                self.hold_df.drop(stop_stks, axis = 0, inplace = True)
            else:
                wts_np[date_idx, self.hold_df.loc[hold_stks, 'index']] = self.stk_pct
    ...
    def NoAdd(self, date_idx, wts_np):
        return wts_np
    def long_N(self):
        return (self.hold_df.loc[:, 'last_pr'] - self.hold_df.loc[:, 'cost']) / self.hold_df.loc[:, 'atr']
    def short_N(self):
        return (self.hold_df.loc[:, 'cost'] - self.hold_df.loc[:, 'last_pr']) / self.hold_df.loc[:, 'atr']
    ...
    def strict_same_direction_pred(self, stk_df):
        ...
        min_cond = self.hold_df['days'] >= self.min_days
        cand_names = self.hold_df[min_cond].index
...
class Trader(nn.Module):
    def __init__(self, hidden_size: int, batch_mom: float, actor_act: nn, critic_act: nn, sector_dim: int, weekday_dim: int, month_dim: int):
        ...
        self.actor_l1 = nn.Linear(sector_dim + weekday_dim + month_dim + 15, hidden_size)
        ...
        self.actor_model = nn.Sequential(self.actor_l1,
            nn.BatchNorm1d(hidden_size, momentum = batch_mom), self.actor_act2, nn.AlphaDropout(p = 0.3), self.actor_l3)
...
if __name__ == '__main__':
    ...
    if not isdir(folder):
        mkdir(folder)
    st_time = time()
    start_date, end_date, prepare_date = '2006-01-01', '2025-05-15', '2005-03-18'
    ...
    dbx_study_fname = f'/qnt_Adamax_lowpct_stop.pkl'
    local_study_fname = f'{folder}{dbx_study_fname}'
    dbx_params_fname = f'/params_Adamax_lowpct_stop.csv'
    local_params_fname = f'{folder}{dbx_params_fname}'
    dbx_df_fname = f'/qnt_Adamax_lowpct_stop.csv'
    local_df_fname = f'{folder}{dbx_df_fname}'
    dbx_wt_fname = f'/wt_Adamax_lowpct_stop.csv'
    local_wt_fname = f'{folder}{dbx_wt_fname}'
    dbx_long_model_fname = f'/long_model_Adamax_lowpct_stop.pt'
    local_long_model_fname = f'{folder}{dbx_long_model_fname}'
    dbx_short_model_fname = f'/short_model_Adamax_lowpct_stop.pt'
