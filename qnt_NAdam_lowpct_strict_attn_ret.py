...
from numpy import nanmedian, delete, isin, concatenate, count_nonzero, unique, dot, corrcoef, log, arange, isnan, nanmax, nanmin, nan, zeros, clip, where, float32, isfinite, array, append, NINF, intersect1d, setdiff1d
...
from torch import exp, from_numpy, topk, inference_mode, save, equal
...
def roll_Shapiro(max_na_cnt, cal_array, window_indices):
    ...
    return DataFrame(array(results_list).T, index = stk_chg_indices, columns = assets) * valid_liq
def wma(df, n):
...
def cal_corr(pr_data, window):
    ...
    result_index = [i - 200 for i in raw_stk_chg_indices]
    def _process_index(idx):
        i = raw_stk_chg_indices[idx]
    ...
    counts_list = Parallel(n_jobs = -1)(
        delayed(_process_index)(i) for i in range(len(raw_stk_chg_indices))
    )
...
def get_features(returns_arr, vol_arr):
    ...
    window_indices = array([arange(i - (roll_size - 1), i + 1) for i in raw_stk_chg_indices])
    ...
    stat_dict['std_lwma5'] = DataFrame(array(wma5_df.loc[stk_select_dates, :]),
                                      index=stk_chg_indices, columns=assets) * valid_liq
    ...
    for period in periods_dict['logret']:
        ...
        stat_dict[f'logret{period}'] = DataFrame(log_total_ret[raw_stk_chg_indices, :], index = stk_chg_indices, columns = assets)
        if period == 10:
    ...
    for period in periods_dict['NATR']:
        ...
        stat_dict[f'NATR{period}'] = DataFrame(array(natr_df.loc[stk_select_dates, :]), index = stk_chg_indices, columns = assets) * valid_liq
    for period in periods_dict['aroon']:
        up, down = xrta.AROON(data, period)
        stat_dict[f'uparoon{period}'] = DataFrame(array(up.to_pandas().loc[stk_select_dates, :]), index = stk_chg_indices, columns = assets) * valid_liq
        if period == 20:
            stat_dict[f'downaroon{period}'] = DataFrame(array(down.to_pandas().loc[stk_select_dates, :]), index = stk_chg_indices, columns = assets) * valid_liq
    for period in periods_dict['money_vol']:
        ...
        stat_dict[f'money_vol{period}'] = DataFrame(array(money_vol_df.loc[stk_select_dates, :]), index = stk_chg_indices, columns = assets) * valid_liq
    for period in periods_dict['BOP']:
        bop_trima = xrta.TRIMA(xrta.BOP(data), period).to_pandas()
        stat_dict[f'BOP_trima{period}'] = DataFrame(array(bop_trima.loc[stk_select_dates, :]), index = stk_chg_indices, columns = assets) * valid_liq
...
def objective(trial):
    ...
    long_act = trial.suggest_categorical("long_act", func_lt)
    short_act = trial.suggest_categorical("short_act", func_lt)
    trader_wt_decay = 10 ** (-trial.suggest_int("trader_wt_decay", 3, 5, step = 1))
    #d_model = 2 ** trial.suggest_int("d_model", 7, 9, step = 1)
    d_model = 256
    n_head = 2 ** trial.suggest_int("n_head", 1, 3, step = 1)
    #n_layer = trial.suggest_int("n_layer", 3, 4, step = 1)
    n_layer = 3
    total_epochs = 36
    stk_cnt_lt = [18, 24, 36, 45]
    ...
    long_trader_lr = 1 / 10 ** trial.suggest_int('long_trader_lr', 2, 4, step = 1)
    short_trader_lr = 1 / 10 ** trial.suggest_int('short_trader_lr', 2, 4, step = 1)
    hold_period = trial.suggest_int('hold_period', 2, 5, step = 1)
    #hold_period = 8
    #update_cnt = trial.suggest_int('update_cnt', 3, 6, step = 1)
    update_cnt = 6
    ...
    longgain_N = trial.suggest_int("long_gain_N", 20, 45, step = 5) / 10
    shortgain_N = trial.suggest_int("short_gain_N", 15, 40, step = 5) / 10
    long_min_days = trial.suggest_int("long_min_days", 60, 180, step = 10)
    short_min_days = trial.suggest_int("short_min_days", 10, 40, step = 5)
    long_lgbm = model_dict['long'][model_type]
    short_lgbm = model_dict['short'][model_type]
    long_eps = 1 / 10 ** 8
    short_eps = 1 / 10 ** 8
    #actor_act: nn, d_model: int, nhead: int, num_layers: int
    long_model = Trader(getattr(nn, long_act), d_model, n_head, n_layer)
    short_model = Trader(getattr(nn, short_act), d_model, n_head, n_layer)
    long_opt = NAdam(long_model.parameters(),
    lr = long_trader_lr, eps = long_eps, weight_decay = trader_wt_decay, decoupled_weight_decay = True, maximize = False, foreach = True)
    short_opt = NAdam(short_model.parameters(),
    lr = short_trader_lr, eps = short_eps, weight_decay = trader_wt_decay, decoupled_weight_decay = True, maximize = False, foreach = True)
    params_dict = {'atr_ma': atr_ma, 'update_cnt': update_cnt,
    ...
    return process(total_epochs, hold_period, stk_pct, params_dict)
def speedtest(function_wrapper, stk_pct, params_dict):
    import cProfile
    import pstats
    import snakeviz.cli as cli
    with cProfile.Profile() as pr:
        function_wrapper(stk_pct, params_dict)
    stats = pstats.Stats(pr)
    stats.sort_stats(pstats.SortKey.TIME)
    filename = f"{folder}/testmodel_profile.prof"
...
def test_model(stk_pct, params_dict):
    ...
    long_model.eval()
    short_model.eval()
    long_opt_model = jit.optimize_for_inference(jit.script(long_model))
    short_opt_model = jit.optimize_for_inference(jit.script(short_model))
    with inference_mode():
        for chg_date in all_tensors_dict.keys():
            ...
            if chg_date == fst_select_date:
                ...
                long_logits = long_opt_model(final_tensor, liq_tensor)
                short_logits = short_opt_model(final_tensor, liq_tensor)
                if long_logits.shape[0] != 1 and short_logits.shape[0] != 1:
                    ...
                    #initial_indices, trade_type, add_type, max_add_cnt, stk_pct, loss_N, gain_N, add_N, min_days
                    LongSys = TradeSystem(new_long_indices, 'long', 'None', 0, stk_pct, longloss_N, longgain_N, 1, long_min_days)
                    ShortSys = TradeSystem(new_short_indices, 'short', 'None', 0, -stk_pct, shortloss_N, shortgain_N, 1, short_min_days)
                ...
                dropped_long_indices, dropped_short_indices = LongSys.drop_dict[chg_date_idx], ShortSys.drop_dict[chg_date_idx]
                #print('Dropped long indices:', dropped_long_indices)
                #print('Dropped short indices:', dropped_short_indices)
                ...
                new_long_cnt, new_short_cnt = len(illiq_long_indices) + stop_long_cnt, len(illiq_short_indices) + stop_short_cnt

                long_logits = long_opt_model(final_tensor, liq_tensor)
                short_logits = short_opt_model(final_tensor, liq_tensor)
                if long_logits.shape[0] != 1 and short_logits.shape[0] != 1:
                    min_long_val, min_short_val = long_logits.min(), short_logits.min()
                    long_logits[all_hold_indices] = min_long_val
                    short_logits[all_hold_indices] = min_short_val
                    long_logits[dropped_long_indices] = min_long_val
                    short_logits[dropped_short_indices] = min_short_val
                    select_params['long']['logits'] = long_logits
                    select_params['short']['logits'] = short_logits
                    select_params['long']['new_cnt'] = new_long_cnt
                    select_params['short']['new_cnt'] = new_short_cnt
...
def selection(select_params):
    ...
    if common_stks.shape[0] > 0:
        ...
        min_long_val, min_short_val = long_logits.min(), short_logits.min()
        if short_vacants > 0:
            short_logits[new_long_indices] = min_short_val
            short_logits[new_short_indices] = min_short_val
            ...
            long_logits[new_short] = min_long_val
        if long_vacants > 0:
            long_logits[new_long_indices] = min_long_val
            long_logits[new_short_indices] = min_long_val
...
class BestTrial():
    def __init__(self):
        ...
        self.stat_time_lt = []
class TradeSystem():
    def __init__(self, initial_indices, trade_type, add_type, max_add_cnt, stk_pct, loss_N, gain_N, add_N, min_days):
        ...
        self.drop_dict = dict()
        for chg_date_idx in stk_chg_indices:
            self.drop_dict[chg_date_idx] = array([], dtype = 'int16')
    def Monitor(self, date_idx, atr_np, prices_np, wts_np):
        chg_date_idx = monitor_reverse_dict.get(date_idx, 0)
        if self.hold_np.shape[0] > 0:
            ...
            if len(stop_stks_row_indices) > 0:
                ...
                self.drop_dict[chg_date_idx] = concatenate((self.drop_dict[chg_date_idx], stop_true_indices))
...
class Trader(nn.Module):
    def __init__(self, actor_act: nn, d_model: int, nhead: int, num_layers: int):
        super(Trader, self).__init__()
        self.err_flag = tones(1)
        self.emb_sector = nn.Embedding(9, 8)
        self.emb_month = nn.Embedding(12, 9)
        self.emb_weekday = nn.Embedding(5, 4)
        self.cont_projection_l1 = nn.Linear(15, d_model - 21)#21=8+9+4
        self.feature_fusion_l2 = nn.Linear(d_model, d_model)
        self.hidden_size = d_model // 2
        self.l3 = nn.Linear(d_model, self.hidden_size)
        ...
        if isinstance(actor_act(), (nn.Sigmoid, nn.Tanh, nn.Softshrink, nn.Softsign, nn.Tanhshrink)):
            nn.init.xavier_uniform_(self.l3.weight)
        else:
            nn.init.kaiming_uniform_(self.l3.weight, nonlinearity = 'leaky_relu')
    def forward(self, state, liq_tensor):
        ...
        continuous_proj = self.cont_projection_l1(numerical_data)  # (n_stocks, d_model-21)
        x = self.feature_fusion_l2(tcat([continuous_proj, sector_, month_, weekday_], dim = 1))  # (batch_size, n_stocks, d_model)
        # Multi-head attention处理
        for attention, layer_norm in zip(self.attention_layers, self.layer_norms):
            # Self-attention
            attn_output = attention(x, x, x, need_weights = False)[0]
            x = layer_norm(tadd(x, self.dropout(attn_output)))
        output = self.output_projection(x)  # (batch_size, n_stocks, 1)
        logits = output.T.squeeze(0)
        logits_liq = twhere(liq_tensor == 0, 0, logits)
        pure_logits = tdiv(logits_liq, logits_liq.sum())
        max_val = pure_logits.max()
        min_val = pure_logits.min()
        #print('max val:', max_val)
        #print('Shape of pure probs:', pure_probs.shape)
        logits_detach = pure_logits.detach()
        if (max_val == 0) or (not tisfinite(logits_detach).all()):
            return self.err_flag
        else:
            return twhere(liq_tensor == 0, min_val, pure_logits)
...
if __name__ == '__main__':
    ...
    if not isdir(folder):
        mkdir(folder)
    st_time = time()
    start_date, end_date, prepare_date = '2006-01-01', '2025-05-22', '2005-03-18'
    ...
    stk_chg_indices = liquid_df.iloc[::10].index.values
    raw_stk_chg_indices = stk_chg_indices + 200
    all_dates = raw_liq_df.index.values
    stk_select_dates = all_dates[stk_chg_indices]
    valid_liq = DataFrame(array(valid_liq.loc[stk_select_dates, :]), index = stk_chg_indices, columns = assets)
    end_time01 = time()
    prepare_time = round((end_time01 - st_time) / 60, 3)
    print('Time to prepare data:', prepare_time, 'min')
    dbx_study_fname = f'/qnt_NAdam_lowpct_strict_attn_ret.pkl'
    local_study_fname = f'{folder}{dbx_study_fname}'
    dbx_params_fname = f'/params_NAdam_lowpct_strict_attn_ret.csv'
    local_params_fname = f'{folder}{dbx_params_fname}'
    dbx_df_fname = f'/qnt_NAdam_lowpct_strict_attn_ret.csv'
    local_df_fname = f'{folder}{dbx_df_fname}'
    dbx_wt_fname = f'/wt_NAdam_lowpct_strict_attn_ret.csv'
    local_wt_fname = f'{folder}{dbx_wt_fname}'
    dbx_long_model_fname = f'/long_model_NAdam_lowpct_strict_attn_ret.pt'
    local_long_model_fname = f'{folder}{dbx_long_model_fname}'
    dbx_short_model_fname = f'/short_model_NAdam_lowpct_strict_attn_ret.pt'
    ...
    dbx_encoder_fname = f'/sector_encoder_sk161.joblib'
    local_encoder_fname = f'{folder}{dbx_encoder_fname}'
    def gumbel_topk(logits, k, tau=1.0):
        uni = trand((k, logits.size(0)))
        # gumbel = -log(-log(U))，形狀 (k, n)
        gumbels = -tlog(-tlog(uni + 1e-20) + 1e-20)
        # 2) 準備一個形狀 (k, n) 的 alpha，每一列一開始都等於原 logits
        #    我們會依序在迴圈裡更新 alpha[j+1] = alpha[j] + log(1 - a_j)
        alpha = logits.unsqueeze(0).repeat(k, 1)  # shape (k, n)
        # 3) mask 用一維累加，就算是 k‐hot 也只留在最後一步傳回
        mask = tzeros_like(logits)           # shape (n,)
        # 4) 迴圈裡只做「取 j-th row 的 Gumbel noise，加到 alpha[j] → gumbel-softmax → one-hot → 更新 mask、更新 alpha[j+1]」
        #    全部運算都用 PyTorch Tensor，不在迴圈內呼叫 torch.rand、F.gumbel_softmax 等抽樣函數
        for j in range(k):
            # 4.1) 先把 alpha[j] + gumbels[j] 做 Gumbel‐Softmax
            #     scores = (alpha[j] + gumbels[j]) / tau
            scores = (alpha[j] + gumbels[j]) / tau
            # 4.2) 先算出「soft」機率 a_soft (可微分)，再做 hard one‐hot (straight‐through)
            a_soft = softmax(scores, dim=0)  # shape (n,)
            # 找最大值 index，製作 one‐hot
            top_idx = targmax(a_soft)
            a_hard = tzeros_like(a_soft)
            a_hard[top_idx] = 1.0
            # straight‐through trick: gradient 走 a_soft，但 forward 用 a_hard
            a = (a_hard - a_soft).detach() + a_soft  # shape (n,)
            # 4.3) 把這一次的 one‐hot 加到 mask 上
            mask = mask + a  # 到最後會累加成 k‐hot
            # 4.4) 如果還有下一輪 (j+1 < k)，就把 alpha[j+1] = alpha[j] + log(1 - a)
            #     讓下一輪抽樣時，同樣這支已經被選過的股票機率趨近 0
            if j + 1 < k:
                # (1 - a) 可能有 a_soft 部分不為零，加 eps 保險
                alpha[j + 1] = alpha[j] + tlog(1.0 - a + 1e-20)
        # 5) 回傳的一維 mask 在 forward 看起來是「k‐hot」(k 個元素向 1 靠攏)，
        #    backward 時透過 a_soft 保持了可微分性
        return mask
    def train(model, opt, trade_type: int, hold_period: int, update_cnt: int, epoch: int, stk_cnt: int, return_dict: dict):
        ...
        #train_cnt = 0
        train_time_lt, gumbel_time_lt = [], []
        #tmp_wts = zeros((row_cnt, assets_cnt))
        tau_init, tau_min, alpha = 1.0, 0.15, 0.95
        accum_counter = 0
        for fst_date in true_train_dates:
            ...
            logits = model(stk_data, liq_tensor)
            if logits.shape[0] != 1:
                gumbel_st_time = time()
                mask = gumbel_topk(logits, k = stk_cnt, tau = max(tau_min, tau_init * (alpha ** epoch)))
                gumbel_end_time = time()
                gumbel_time_lt.append(gumbel_end_time - gumbel_st_time)
                mon_liq = liquid_arr[start_idx: end_idx, :]
                stock_returns = pct_tensor[start_idx: end_idx, :] * trade_type * mon_liq
                true_daily_rets = stock_returns * mask / stk_cnt
                mean_ret = (true_daily_rets.sum(dim = 1)).mean()
                #std_ret  = (true_daily_rets.sum(dim = 1)).std(unbiased = False) + 1e-6
                #sharpe = mean_ret / std_ret
                loss = -mean_ret / update_cnt
                loss.backward()  # 累積梯度
                accum_counter += 1
                if accum_counter == update_cnt:
                    clip_grad_norm_(model.parameters(), max_norm = 5, norm_type = 2.0, error_if_nonfinite = True)
                    opt.step()  # 更新參數
                    opt.zero_grad()  # 重置梯度
                    accum_counter = 0
                #train_cnt += 1
            ...
            if not stop:
                ...
                with ctx.Manager() as manager:
                    ...
                    processes = [
                        ctx.Process(
                            target = train,
                            args = (long_model, long_opt, 1, hold_period, update_cnt, epoch, stk_cnt, return_dict)
                        ),
                        ctx.Process(
                            target = train,
                            args = (short_model, short_opt, -1, hold_period, update_cnt, epoch, stk_cnt, return_dict)
                        )
                    ]
                    ...
                    if type(long_result) != int and type(short_result) != int:
                        ...
                        stop = True
                        break
        #speedtest(test_model, stk_pct, params_dict)
    ...
    st_time02 = time()
    date_idx_dict = dict(zip(stk_select_dates, stk_chg_indices))
    ...
    monitor_dict, monitor_reverse_dict = dict(), dict()
    chg_date = stk_select_dates[0]
    for next_chg_date in stk_select_dates[1:]:
        monitor_dates = all_dates[(all_dates > chg_date) & (all_dates < next_chg_date)]
        monitor_dict[chg_date] = monitor_dates
        for monitor_dt_ in monitor_dates:
            monitor_reverse_dict[date_indices[monitor_dt_]] = date_indices[next_chg_date]
    ...
    batch, input_col_len, func_lt = len(assets), 18, ['ELU', 'GELU', 'CELU', 'Sigmoid', 'SiLU', 'Mish', 'Softplus', 'Softshrink', 'Softsign', 'Tanhshrink', 'Tanh']
    ...
    if download_val != 1:
        ...
        best_trial_num, trial_cnt = study.best_trial.number, 15
    ...
    print('--------Median gumbel time:', med_stat_time, 'min--------')
    #print('---Correlation---')
